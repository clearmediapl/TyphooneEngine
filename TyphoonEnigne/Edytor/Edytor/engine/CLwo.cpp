...
void CLwo::Load(int size, char *dane)
{
	...
	if(!defaultPath)
	{
			...
			memset(obj->texture_name, 0, sizeof(obj->texture_name));
			
	
			strcpy(obj->texture_name,dane+i);//read all path
	
			// add / to name of disc
			for(i=sizeof(obj->texture_name);i>0;i--)
			{
				obj->texture_name[i+1] = obj->texture_name[i];
			}
			obj->texture_name[2] = '/';
	}
	else
...
void CLwo::CreateCollisionTree(NewtonWorld *nWorld, NewtonTreeCollisionCallback levelCallbac)
{
